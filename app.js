...
app.all('/psychometric', upload.array(), function (req, res, next) {
  if (req.body.length == 0) {
    ...
    var querystring = "INSERT INTO PSYCHOMETRIC_AGG_SCORE (USERID,QUESTION_ID,QUESTION,ANSWER_ID,ANSWER) VALUES ?";
...
app.all('/psychometricScoreAdmin', upload.array(), function (req, res, next) {
  ...
  var querystring = "SELECT PSYCHOMETRIC_AGG_SCORE.USERID,SUM(PSYCHOMETRIC_ANSWERS.ANSWER_SCORE) AS NET_SCORE FROM  PSYCHOMETRIC_AGG_SCORE INNER JOIN PSYCHOMETRIC_ANSWERS ON PSYCHOMETRIC_AGG_SCORE.ANSWER_ID = PSYCHOMETRIC_ANSWERS.ANSWER_ID GROUP BY  PSYCHOMETRIC_AGG_SCORE.USERID;";
  connection.query(querystring,function (err, result) {
    if (err) {
      res.json({ "result": "failure" });
      throw err
